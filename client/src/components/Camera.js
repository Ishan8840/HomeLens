...
const FullscreenCamera = () => {
  ...
  const enableOrientation = async () => {
    const handleOrientation = (event) => {
      ...
      if (now - lastUIUpdate.current > 150) {
        ...
        // Use the state setter to satisfy the "unused var" rule and gate logic
  ...
  useEffect(() => {
    if (!isStarted) return;
    // Don't run logic until sensors are ready (fixes unused var warning too)
  ...
  }, [isStarted, hasOrientationFix]); // Added hasOrientationFix to dependencies
